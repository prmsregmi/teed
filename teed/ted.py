...
from .activation import smish as Fsmish
from .activation import Smish
...
class DoubleConvBlock(nn.Module):
    ...
    def forward(self, x):
        # print()
        # print_min_max("Input", x)
        x = self.conv1(x)
        # print_min_max("First Conv", x)
        x = self.smish(x)
        # print_min_max("First Activation", x)
        x = self.conv2(x)
        # print_min_max("Second Conv", x)
        if self.use_act:
            x = self.smish(x)
            # print_min_max("Second Activation", x)
        return x
